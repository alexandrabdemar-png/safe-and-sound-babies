...
//
// This is the fallback path used on web and Android, and on iOS when
// VisionKit's native scanner isn't available (pre-iOS 16). See
// BarcodeScannerView.tsx for the platform dispatcher that picks between
// this and the native VisionKit scanner.
const SCAN_FORMATS = [
...
export type BarcodeScannerViewProps = {
  ...
  /** Called when the user explicitly cancels/backs out of the scanner, if the active scanner implementation supports reporting that separately from an error. */
  onCancel?: () => void;
...
export function WebBarcodeScannerView({
  onDetected,
  onError,
  active = true,
  className,
