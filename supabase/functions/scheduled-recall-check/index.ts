...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const productIds = [...new Set(resolvedMatches.map((m) => m.product_id))];
    const { data: existingRows } = productIds.length
      ? await supabase
          .from("product_recalls")
          .select("product_id, recall_id")
          .in("product_id", productIds)
      : { data: [] as Array<{ product_id: string; recall_id: string }> };
    const existingKeys = new Set((existingRows ?? []).map((r) => `${r.product_id}:${r.recall_id}`));
    const newMatches = resolvedMatches.filter(
      (m) => !existingKeys.has(`${m.product_id}:${m.recall_id}`),
    );
    ...
    const matchedProductIds = [...new Set(resolvedMatches.map((m) => m.product_id))];
    ...
    // ── Notify only users with a genuinely new match ─────────────────────
    ...
    console.error("[scheduled-recall-check] failed:", e instanceof Error ? e.message : String(e));
