...
import { ClipboardList, Copy, Printer, Check, Share2 } from "lucide-react";
...
export const Route = createFileRoute("/_authenticated/caregiver-card")({
  ...
  head: () => ({ meta: [{ title: "Caregiver Card — Peace of Mine" }] }),
});
...
function CaregiverCardPage() {
  ...
  function buildCardText() {
    ...
    const dobStr = child.date_of_birth
      ? new Date(child.date_of_birth + "T00:00:00").toLocaleDateString(undefined, { year: "numeric", month: "long", day: "numeric" })
      : "unknown";
    const contactLines = contacts.length > 0
      ? contacts.map((c) => `${c.name}${c.relationship ? ` (${c.relationship})` : ""}: ${c.phone}`)
      : ["None on file"];
    return [
      "PEACE OF MINE — CAREGIVER CARD",
      `Child: ${child.name}, born ${dobStr}`,
      `Parent phone: ${card.parentPhone || "—"}`,
      `Pediatrician: ${card.pediatricianName || "—"} — ${card.pediatricianPhone || "—"}`,
      "",
      ...
      "",
      "ROUTINES",
      card.bedtimeRoutine ? `Bedtime: ${card.bedtimeRoutine}` : "",
      card.napRoutine ? `Nap: ${card.napRoutine}` : "",
      card.feedingNotes ? `Feeding: ${card.feedingNotes}` : "",
      "",
      "Generated by Peace of Mine",
  ...
  async function shareCard() {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: `Caregiver Card — ${child?.name}`, text });
        return;
      } catch {
      ...
      <main className="flex-1 px-5 sm:px-6">
        <div className="mx-auto max-w-md space-y-6">
          ...
          >
            ...
            {/* Parent phone */}
            <div className="mb-5">
              <h3 className="mb-2 font-display text-base font-semibold text-foreground">Parent's phone</h3>
              <input
                ...
                type="tel"
                value={card.parentPhone}
                onChange={(e) => updateField("parentPhone", e.target.value)}
              />
              <p className="hidden font-body text-sm print:block">{card.parentPhone || "—"}</p>
            </div>
