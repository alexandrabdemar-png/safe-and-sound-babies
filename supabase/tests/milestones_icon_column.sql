...
-- has_child_access, from 20260708000000_caregiver_access_rls_hardening.sql)
-- still isolates rows per-child after the ALTER TABLE.
...
-- ── All 4 currently-valid icon values are accepted and round-trip ────────
SELECT test.login('authenticated', 'a1111111-1111-1111-1111-111111111111');
...
SELECT test.login('service_role');
SELECT test.assert(
  (SELECT icon FROM public.milestones WHERE child_id = 'c1111111-1111-1111-1111-111111111111' AND title = 'Star moment') = 'star',
  'Adversarial: a stranger''s UPDATE of another child''s milestone icon silently affected 0 rows — value is still ''star'''
);
SELECT test.logout();
